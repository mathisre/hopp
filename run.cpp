...
void run::runCurrentTrace(class params *p) //2D only so far!!!
{
//  setlocale(LC_ALL,"");
...
//  fflush(stdout);
  ...
  FILE* f;
  string filename = "positions.dump";
  MKcurr->initPositions(filename);
  ...
  for (run = 0; run < runs; run++)
  {
    ...
    MKcurr->runCurrentTrace(steps, energy, t, filename);
    printf("\n-------Finished with run %d--------\n\n", run);
    printf("For fil\n");
    fflush(stdout);
